...
- `great-britain-latest.tar.gz` — graph files for `osrm-routed --algorithm mld` covering England, Scotland, Wales. Built from the Geofabrik `great-britain-latest.osm.pbf` snapshot using the `car.lua` profile.
...
Point your OSRM data volume at a release asset:

```sh
OSRM_PREBUILT_URL=https://github.com/neofenyx/osrm-data/releases/download/latest/great-britain-latest.tar.gz
curl -L --fail "$OSRM_PREBUILT_URL" | tar xz -C /data
